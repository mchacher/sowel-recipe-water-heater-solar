 ...
 */
export function readDraw(bindings, now = Date.now()) {
    const b = bindings?.find((x) => x.category === "power") ??
    ...
    if (!Number.isFinite(at) || now - at > DRAW_FRESH_MS)
        return null;
    return b.value;
}
    ...
    return {
        ...
        createInstance(params, ctx) {
            ...
            const watchIdle = () => {
                ...
                const draw = readDraw(ctx.equipmentManager.getDataBindingsWithValues?.(heaterId));
                if (draw === null)
                    return; // unknown, never idle
                ...
                if (now - idleSince < IDLE_RELEASE_MS)
                    return;
                ctx.log(`${heaterName} ne consomme plus depuis ${Math.round(IDLE_RELEASE_MS / 60_000)} min -> surplus rendu à l'arbitre`);
                try {
                    claim.release();
                }
                catch {
